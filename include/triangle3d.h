...
public:
			...
			const vector3d<T> &lineVect, vector3d<T> &outIntersection) const
	{
		...
		core::triangle3d<irr::f64> trianglef64(vector3d<f64>((f64)pointA.X, (f64)pointA.Y, (f64)pointA.Z), vector3d<f64>((f64)pointB.X, (f64)pointB.Y, (f64)pointB.Z), vector3d<f64>((f64)pointC.X, (f64)pointC.Y, (f64)pointC.Z));
...
private:
			...
			const vector3d<f64> &a, const vector3d<f64> &b) const
	{
		...
		if (res < 0) {
			...
			if (core::iszero(cp1n.X, (f64)ROUNDING_ERROR_f32) && core::iszero(cp1n.Y, (f64)ROUNDING_ERROR_f32) && core::iszero(cp1n.Z, (f64)ROUNDING_ERROR_f32)) {
				res = 0.f;
			}
		}
		return (res >= 0.0f);
