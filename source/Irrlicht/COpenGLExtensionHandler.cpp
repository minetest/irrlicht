...
COpenGLExtensionHandler::COpenGLExtensionHandler() :
		...
		,
		pGlActiveTexture(0), pGlActiveTextureARB(0), pGlClientActiveTextureARB(0),
...
#define tmplog(x, y) os::Printer::log(x, core::stringc(y).c_str())
				...
				++j;
				tmplog("Acceleration ", (vals[j] == WGL_NO_ACCELERATION_ARB ? "No" : vals[j] == WGL_GENERIC_ACCELERATION_ARB ? "Generic"
																			 : vals[j] == WGL_FULL_ACCELERATION_ARB			 ? "Full"
																															 : "ERROR"));
				tmplog("Need palette ", vals[++j]);
				tmplog("Need system palette ", vals[++j]);
				tmplog("Swap layer buffers ", vals[++j]);
				++j;
				tmplog("Swap method ", (vals[j] == WGL_SWAP_EXCHANGE_ARB ? "Exchange" : vals[j] == WGL_SWAP_COPY_ARB ? "Copy"
																				: vals[j] == WGL_SWAP_UNDEFINED_ARB	 ? "Undefined"
																													 : "ERROR"));
